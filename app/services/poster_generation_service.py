...
class VisionPosterDecision(BaseModel):
    selected: bool = Field(
        description=(
            "Whether this image should be selected for poster generation. "
            "Select true only if it is relevant to the news and has no visible text/watermark."
        )
    )

    rejection_reason: Optional[str] = Field(
        default=None,
        description="If selected=false, explain clearly why this image was rejected."
    )

    overlay_color: str = Field(
        default="#071225",
        description="Dark overlay/base color for poster gradient. Use HEX color."
    )

    headline_big_color: str = Field(
        default="#ffffff",
        description="Color for the big headline line. Use HEX color."
    )

    headline_small_color: str = Field(
        default="#57d8ff",
        description="Color for the smaller headline line. Use HEX color."
    )
    ...
    font_family: Literal[
        ...
        default="Montserrat",
        description="Professional sports poster font family."
    )

    headline_big_font_size: Optional[int] = Field(
        default=86,
        description="Font size in px for the big headline line."
    )

    headline_small_font_size: Optional[int] = Field(
        default=56,
        description="Font size in px for the small headline line."
    )
...
def _get_retrieved_items_from_state(state: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Keeps the retrieval relevancy order.
    The first chunk/item is assumed to be most relevant.
    """
...
def extract_ranked_image_candidates(
    ...
    """
    Does only 2 things:
    1. Extract image URLs from retrieved chunks/items in relevancy order.
    2. Skip images rejected by user feedback.
    """
    ...
    for item in items:
        ...
        # Direct raw chunk context. No extra context helper.
...
def download_ranked_images(
    ...
    """
    Downloads ranked images into app/poster_images/rank1.jpg, rank2.jpg...
    Does NOT delete other ranked images.
    """
    ...
    for item in ranked_candidates:
        ...
        try:
            ...
            print(
                f"[poster_generation] failed to download rank{rank}: "
                f"{url} | {e}"
            )
...
Retrieved raw chunk connected to this image:
...
Reject this image if:
- the image appears unrelated to the player, team, match, country, club, or event in the news context
- the image is a generic football photo with no clear relation to the news
- the image shows a different main team/player/event than the news context
- the raw chunk context does not support using this image

Accept only if:
- the image appears relevant to the main football subject, team, player, country, match, or event
- or the raw retrieved chunk context strongly suggests the image belongs to the same news item

PART 2 — IMAGE QUALITY CHECK

Reject this image if:
- it has visible text
- it has a watermark
- it looks pre-edited, posterized, or already designed for social media

Allowed:
- cropped image if there is no visible text
- collage image if there is no visible text
- raw football editorial/action/celebration photo

PART 3 — DESIGN DECISION

If selected=true, decide these design values:

overlay_color:
- choose a dark, high-contrast overlay/base color from the image
- must be a HEX color
- examples: #071225, #050816, #111827, #0b1220

headline_big_color:
- analyze the image colors and overlay_color
- choose a color that creates strong contrast and stands out immediately
- prioritize readability on mobile devices
- avoid colors already dominant in the image
- if no suitable color exists, use #FFFFFF
- return only a HEX color

headline_small_color:
- choose a bright accent color that contrasts well with the image
- examples: #57d8ff, #38bdf8, #60a5fa, #facc15, #22d3ee
...
- choose between 78 and 96

headline_small_font_size:
- choose between 44 and 62

If selected=false:
- still return default design values
- write rejection_reason clearly
...
def _default_poster_decision() -> Dict[str, Any]:
    return {
        "selected": False,
        "rejection_reason": "Vision analysis failed or all images were rejected. Fallback design used.",
        "overlay_color": "#071225",
        "headline_big_color": "#ffffff",
        "headline_small_color": "#57d8ff",
        "font_family": "Montserrat",
...
def select_best_image_sequentially(
    ...
    """
    Sends images to Groq Vision in rank order.

    If rank1 is selected:
        stop immediately
        keep rank1

    If rank1 is rejected:
        try rank2
        continue until one image is selected

    Important:
    This function does NOT delete any ranked images.
    """
...
def generate_split_headlines(
    ...
    structured_llm = llm.with_structured_output(
        PosterHeadlineOutput
    )

    font_family = poster_design.get("font_family") if poster_design else "Montserrat"
...
- use ASCII English letters only
- no Cyrillic or non-English lookalike characters
...
"""
    ...
    # small cleanup for common LLM weird character issue
    headline_big = headline_big.replace("М", "M").replace("е", "e")
    headline_small = headline_small.replace("М", "M").replace("е", "e")

    return {
        "headline_big": headline_big,
...
def prepare_poster_assets(selected_image_path: str) -> Dict[str, str]:
    """
    Copies selected poster image into app/poster_generation/background.jpg.
    Copies logo into app/poster_generation/logo.png if app/assets/logo.png exists.
    """
...
def generate_poster_html_css(
    ...
    """
    Fixed premium template.
    No LLM creates HTML/CSS.
    CSS is dynamically adjusted using Groq Vision design values.
    """

    overlay_color = poster_design.get("overlay_color") or "#071225"
    headline_big_color = poster_design.get("headline_big_color") or "#ffffff"
    headline_small_color = poster_design.get("headline_small_color") or "#57d8ff"
    font_family = poster_design.get("font_family") or "Montserrat"
    headline_big_font_size = int(poster_design.get("headline_big_font_size") or 86)
    headline_small_font_size = int(poster_design.get("headline_small_font_size") or 56)
...
<head>
    ...
    <title>Football Insider Poster</title>

    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=League+Spartan:wght@700;800;900&family=Oswald:wght@600;700&family=Bebas+Neue&family=Montserrat:wght@700;800;900&display=swap" rel="stylesheet">
...
/* Strong dark cinematic bottom blend */
...
h1{{
    ...
    text-transform:uppercase;
    margin-bottom:18px;
    text-shadow:0 7px 22px rgba(0,0,0,.32);
    max-width:94%;
}}
...
def render_poster_image(html_path: str) -> str:
    """
    Renders index.html into app/poster_generation/poster.png.

    Important:
    We run Playwright in a separate thread because the main app
    is already running inside an asyncio loop from FastAPI/Telegram.
    """
...
def run_poster_image_pipeline(
    ...
    poster_image_dir = ensure_poster_image_dir()
    POSTER_GENERATION_DIR.mkdir(parents=True, exist_ok=True)
    ...
    if not ranked_candidates:
        raise ValueError("No image URLs found from retrieved chunks.")

    print(f"[poster_generation] ranked image candidates: {len(ranked_candidates)}")

    downloaded_images = download_ranked_images(
        ranked_candidates
    )

    if not downloaded_images:
        raise ValueError(
            "No image could be downloaded from the extracted image URLs."
        )

    selection_result = select_best_image_sequentially(
    ...
    assets = prepare_poster_assets(
        selected_image_path=selected_image["local_path"]
    )
    ...
    return {
        "poster_image_dir": poster_image_dir,
